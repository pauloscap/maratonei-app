...
export default function FilmesPage() {
  ...
  function Secao({titulo, cor, qtd, subtitulo, idSecao, children}){
    ...
    useEffect(()=>{
      const saved = localStorage.getItem(userId+":sec-"+idSecao)
      if(saved==="1") setExpandido(true)
    },[userId])
    function toggleExpand(){
      const novo =!expandido
      setExpandido(novo)
      localStorage.setItem(userId+":sec-"+idSecao, novo?"1":"0")
    }
    ...
    return <div style={{marginTop:24}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          {subtitulo && <span style={{fontSize:10, opacity:0.35, marginLeft:6}}>{subtitulo}</span>}
        </div>
        {precisaBotao && (
      ...
      {qtd===0? (
        <div style={{background:"#12182F", border:"1px dashed rgba(255,255,255,0.12)", borderRadius:12, padding:"18px 14px", textAlign:"center"}}>
          <div style={{fontSize:11, opacity:0.35}}>Nenhum filme em {titulo.toLowerCase()}</div>
        </div>
      ) : view==="grade"? <div className="grid">{visiveis}</div> : <div className="list">{expandido? listaFlat : listaFlat.slice(0,9)}</div>}
    </div>
  }
...
@media (min-width: 1100px){.grid{grid-template-columns:repeat(6,1fr);gap:18px} }
@media (min-width: 1400px){.grid{grid-template-columns:repeat(7,1fr);gap:20px} }
      `}</style>
