...
export default function FilmesPage() {
  ...
  useEffect(function(){
    ...
    const t = setTimeout(async function(){
      try {
        // 1 - Busca principal no TMDB em portugues
        const url = "https://api.themoviedb.org/3/search/movie?api_key=" + TMDB_KEY + "&language=pt-BR&query=" + encodeURIComponent(busca) + "&include_adult=false"
        const r = await fetch(url)
        const j = await r.json()
        let lista = []
        if (j.results && j.results.length) {
          lista = j.results.slice(0,10).map(function(m){
            return { id: String(m.id), titulo: m.title + (m.release_date? " (" + m.release_date.slice(0,4) + ")" : ""), img: m.poster_path? TMDB_IMG + m.poster_path : "https://picsum.photos/seed/" + m.id + "/400/600" }
          })
        }
        // 2 - Se nao achar, tenta sem acento e em ingles
        if (!lista.length) {
          const r2 = await fetch("https://api.themoviedb.org/3/search/movie?api_key=" + TMDB_KEY + "&query=" + encodeURIComponent(busca))
          const j2 = await r2.json()
          if (j2.results) lista = j2.results.slice(0,10).map(function(m){ return { id: String(m.id), titulo: m.title, img: m.poster_path? TMDB_IMG + m.poster_path : "https://picsum.photos/seed/" + m.id + "/400/600" } })
        }
        // 3 - Fallback OMDb para casos raros
        if (!lista.length) {
          const r3 = await fetch("https://www.omdbapi.com/?apikey=thewdb&s=" + encodeURIComponent(busca) + "&type=movie")
          const j3 = await r3.json()
          if (j3.Search) lista = j3.Search.slice(0,6).map(function(it){ return { id: it.imdbID, titulo: it.Title, img: it.Poster!=="N/A"? it.Poster : "" } })
        }
        setResultados(lista)
   ...
   .list{ display:grid; gap:8px; }.card{ cursor:pointer; }.poster{ width:100%; aspect-ratio:2/3; border-radius:12px; overflow:hidden; background:#12182F; border:1px solid #222b5a; position:relative; }.poster img{ width:100%; height:100%; object-fit:cover; }.badge{ position:absolute; top:6px; left:6px; background:#FFD400; color:#000; font-size:8px; font-weight:900; padding:3px 6px; border-radius:6px; }.tit{ font-size:12px; font-weight:700; margin-top:6px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }.row{ display:flex; gap:12px; padding:10px; background:#12182F; border:1px solid #222b5a; border-radius:12px; cursor:pointer; }.row img{ width:52px; height:78px; border-radius:8px; object-fit:cover; }
      ...
      <div style={{ maxWidth:1280, margin:"0 auto", padding:14, position:"relative" }}>
        <div style={{ background:"#121A3A", border:"1px solid #2a3566", borderRadius:999, display:"flex", alignItems:"center", padding:"0 14px", height:42, maxWidth:420, margin:"0 auto" }}>
          <input value={busca} onChange={function(e){ setBusca(e.target.value) }} placeholder="Buscar filme (ex: devoradores de estrelas)" style={{ flex:1, background:"transparent", border:0, outline:"none", color:"#fff", fontSize:13 }} />
        ...
        {busca && <div style={{ position:"absolute", top:62, left:14, right:14, maxWidth:420, margin:"0 auto", background:"#12182F", border:"1px solid #2a3566", borderRadius:12, zIndex:50, overflow:"hidden" }}>
          ...
          {resultados.length===0 && <div style={{ padding:12, fontSize:12, opacity:0.5 }}>Buscando no TMDB... Se nao achar tente sem acento: devoradores de estrelas</div>}
        </div>}
