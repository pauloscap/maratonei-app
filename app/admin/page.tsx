    ...
    return (
      <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center p-4">
        <div className="bg-white/[0.04] border border-white/10 rounded-2xl p-8 w-full max-w-sm">
          ...
          />
          <button onClick={handleLogin} className="w-full bg-[#FFD400] text-black font-bold py-3 rounded-xl">
            Entrar
          </button>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-2">
            <button onClick={fetchUsers} className="bg-white/10 px-4 py-2 rounded-xl text-sm hover:bg-white/15 transition">↻ Atualizar</button>
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
          <h2 className="font-semibold mb-4">Últimos usuários ({users.length}) {loading && <span className="text-white/40 font-normal">• carregando...</span>}</h2>
          <div className="space-y-2 max-h- overflow-auto pr-2">
            {users.map(u => (
              <div key={u.id} className="flex items-center justify-between bg-white/[0.03] hover:bg-white/[0.05] transition p-3 rounded-xl">
                <div className="flex items-center gap-3">
                  {u.avatar_url? (
                    <img src={u.avatar_url} className="w-9 h-9 rounded-full object-cover" alt="" />
                  ) : (
                    <div className="w-9 h-9 rounded-full bg-[#FFD400] text-black flex items-center justify-center font-bold text-xs">
                      {(u.nome || "?")[0].toUpperCase()}
                    </div>
                  )}
                  <div>
                    <div className="text-sm font-medium">{u.nome || "Sem nome"}</div>
                    <div className="text-xs text-white/40">{u.id.slice(0,8)} {u.username? `• @${u.username}` : ""}</div>
                  </div>
                </div>
