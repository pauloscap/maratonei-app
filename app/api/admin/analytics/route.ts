...
export async function GET(){
  ...
  const { data: allSeries } = await supabase.from('series').select('id,titulo,generos')
  ...
  allSeries?.forEach((s:any)=> serieMap.set(s.id, s))

  // TOP 5 (pra manter seu card de cima igual antes)
  const countTop:any = {}
  watch?.forEach((w:any)=>{
    const t = serieMap.get(w.serie_id)?.titulo || 'Sem título'
    countTop[t]=(countTop[t]||0)+1
  })
  ...
  const gen:any = {}
  watch?.forEach((w:any)=>{
    const generos = serieMap.get(w.serie_id)?.generos || []
    generos.forEach((g:string)=> { if(g!=='Sem categoria') gen[g]=(gen[g]||0)+1 })
  })
  const generosMaisVistos = Object.entries(gen).map(([genero,total])=>({genero,total})).sort((a:any,b:any)=>b.total-a.total).slice(0,12)
  // se ainda tiver tudo Sem categoria, mostra pelo menos o que tem
  if(generosMaisVistos.length===0){
    const fallback:any = {}
    watch?.forEach((w:any)=>{
      (serieMap.get(w.serie_id)?.generos||[]).forEach((g:string)=> fallback[g]=(fallback[g]||0)+1)
    })
    Object.entries(fallback).forEach(([k,v]:any)=> gen[k]=v)
  }

  // TODOS filmes e séries separados (os que estão funcionando)
