...
// NOVO: busca multi = filmes + séries
export async function buscarTMDB(v, setR) {
  if (v.length < 2) { setR([]); return }
  try {
    const key = process.env.NEXT_PUBLIC_TMDB_KEY
    const r = await fetch(
      `https://api.themoviedb.org/3/search/multi?api_key=${key}&language=pt-BR&query=${encodeURIComponent(v)}&include_adult=false`
    )
    const j = await r.json()
    const fil = (j.results || []).filter(x => x.media_type === "movie" || x.media_type === "tv").slice(0, 8)
    setR(fil)
  } catch { setR([]) }
}
...
export async function addSerie(item, setS, S, setM, setP, router, go) {
  try {
    ...
    let { data: e } = await supa.from("series").select("*").eq("tmdb_id", realId).maybeSingle()
    if (!e) {
      const n = {
        tmdb_id: realId,
        titulo: item.name || item.title,
        poster: item.poster_path,
        ano: (item.first_air_date || item.release_date)? new Date(item.first_air_date || item.release_date).getFullYear() : null
      }
